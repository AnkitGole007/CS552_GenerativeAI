...
def fc_ae_loss(reconstructed, x):
    return F.binary_cross_entropy(reconstructed, x, reduction='mean')
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in dataloader:
            ...
            loss = fc_ae_loss(reconstructed, x)
            loss.backward()
            optimizer.step()
    ...
    torch.save(model.state_dict(), "models/fc_ae_model.pth")
    ...
    for i, ax in enumerate(axes.flat):
        ...
        ax.axis('off')
    plt.tight_layout()
    plt.savefig("outputs/fc_ae_generated_faces.png", dpi=300)
    plt.show()
